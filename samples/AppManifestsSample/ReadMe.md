...
The sample `NuGet.config` adds `../../Artifacts` as a local source and the sample project uses `Mobile.BuildTools.AppManifests` version `2.1.0-pre.*` by default so it resolves the package built from this checkout. CI overrides `MobileBuildToolsAppManifestsVersion` with the exact package version generated by the package build.
